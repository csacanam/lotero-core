 ...
 *   player: address that receives the spin (default: payer address)
 *   url: agent base URL (default: http://localhost:4021)
 *   If only one arg and it starts with http, it's treated as url.
 * Env: PAYER_PRIVATE_KEY (wallet that pays, must have USDC on Base)
 */
...
async function main() {
  ...
  const player =
    arg2 && !arg2.startsWith("http") ? arg2 : signer.address;
  ...
  if (res.status === 402) {
    ...
    if (paymentRequired) {
      try {
        ...
        console.log("PAYMENT-REQUIRED header:", paymentRequired.slice(0, 80) + "...");
      }
    } else {
      console.log("(No PAYMENT-REQUIRED header – payment may have been sent but rejected)");
    }
    if (body?.error || body?.details) {
      console.log("Error from server:", body.error || body.details);
    }
    console.log("");
    console.log("Common 402 causes: PAYER wallet needs USDC on Base; PAY_TO must be valid; CDP_API_KEY_ID/SECRET must be set; payer address must pass KYT/OFAC checks.");
    process.exit(1);
  }
